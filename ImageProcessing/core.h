...
#define MIN_DISTANCE 5 //min distance between 2 segments,the threshold to decide whether merge them or not
...
void transToHVSegments(vector<Segment>& roughSegs){
	int thresh = 10;
	vector<Segment> formatSegs;
	Segment seg;
	for(int i=0;i<roughSegs.size();i++){
		if( abs(roughSegs[i].startp.x - roughSegs[i].endp.x) <= thresh ){
			seg.startp = Point( (roughSegs[i].startp.x+roughSegs[i].endp.x)/2,roughSegs[i].startp.y);
			seg.endp = Point( (roughSegs[i].startp.x+roughSegs[i].endp.x)/2,roughSegs[i].endp.y);
			formatSegs.push_back(seg);
		}
		if( abs(roughSegs[i].startp.y - roughSegs[i].endp.y) <= thresh ){
			seg.startp = Point( roughSegs[i].startp.x,(roughSegs[i].startp.y + roughSegs[i].endp.y)/2 );
			seg.endp = Point(roughSegs[i].endp.x,(roughSegs[i].startp.y + roughSegs[i].endp.y)/2 );
			formatSegs.push_back(seg);
		}
	}
			roughSegs = formatSegs;
}
...
boolean isSameLine(Segment& a,Segment& b){
	Point p11 = Point(a.startp.x,a.startp.y);
	Point p12 = Point(a.endp.x,a.endp.y);

	Point p21 = Point(b.startp.x,b.startp.y);
	Point p22 = Point(b.endp.x,b.endp.y);
...
void drawRectangle(Mat& src, Rec& rect, Scalar sclr, double width){
	Point pt = rect.center;

	drawcross(src,pt,Scalar(0,0,255),width);
	line( src, rect.s1.startp, rect.s1.endp,sclr, width, CV_AA);
	line( src, rect.s2.startp, rect.s2.endp, sclr, width, CV_AA);
	line( src, rect.s3.startp, rect.s3.endp, sclr, width, CV_AA);
	line( src, rect.s4.startp, rect.s4.endp, sclr, width, CV_AA);
...
Point findIntersectionSeg2Seg(Segment& s1,Segment& s2){
	...
	if(s1.startp.y == s1.endp.y)//horizontal
		...
		//when 2 segments are intersected
