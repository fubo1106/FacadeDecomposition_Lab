...
/*decomposition code*/
void CImageProcessingDlg::doDecomposition(CString filename){
	string name = filename.GetBuffer(0);
	Mat src = imread(name, 1);
	...
	vector<Rec> rects;
	vector<Segment> segs,verhorizon_segs,nolapping_segs,reult_seg;
	//HoughLinesP(dst, lines, 1, CV_PI/180, 20, MIN_LENGTH, 20 );
	...
	Segments2Vec4i(nolapping_segs,nolapping_lines);
	segmentsTolines(nolapping_lines,&partion_lines);
	Vec4i2Segments(partion_lines,reult_seg);

	transToHVSegments(reult_seg);

	symmetricCompletion(src,reult_seg,rects);
	...
	ShowMatImgToWnd(GetDlgItem(IDC_FacadeImg), src);
